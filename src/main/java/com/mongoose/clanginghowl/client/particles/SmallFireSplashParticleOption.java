...
public class SmallFireSplashParticleOption implements ParticleOptions {
   public static final MapCodec<SmallFireSplashParticleOption> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
           Codec.FLOAT.fieldOf("size").forGetter(d -> d.size),
           Codec.INT.fieldOf("speed").forGetter(d -> d.speed)
   ).apply(instance, SmallFireSplashParticleOption::new));
   public static final StreamCodec<FriendlyByteBuf, SmallFireSplashParticleOption> STREAM_CODEC = StreamCodec.of(
           (buffer, option) -> option.writeToNetwork(buffer),
           buffer -> new SmallFireSplashParticleOption(buffer.readFloat(), buffer.readInt()));
